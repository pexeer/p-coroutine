...
public:
    ...
    TaskHandle *pop() { return task_queue_.pop(); }

    size_t steal(TaskHandle *item[], size_t max_size) { return task_queue_.steal(item, max_size); }

    size_t steal_task(TaskHandle *item[], size_t max_size);

    void push_back(TaskHandle *task) { task_queue_.push_back(task); }

    TaskHandle *main_task() { return &main_task_; }
    ...
    TaskHandle *cur_task() const { return cur_task_; }
...
public:
    static bool running_in_worker() { return tls_w != nullptr; }
